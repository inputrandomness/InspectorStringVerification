...
A Godot inspector plugin for live-filtering `Strings` or `Stringnames` when edited in the inspector, along with multiple options for filling the list of valid strings. Provides a check-mark when a value is verified, but does not prevent entering invalid values.

## Usage and Behavior
When editing a resource in the inspector with exported `Strings` or `StringNames`, this add-on opens a live-filtered dropdown list in a popup panel for viewing a predetermined list of valid strings. Double-clicking, or using up/down arrows and hitting enter or space, fills the edit box with the selected string. Shows a verification check-mark when the value in the property is in the valid list.
...
Default behavior is provided through a provided "filler" class which may can either use a "directory" file to track and select string lists or have the list provided directly within the calling object. The different usages are control on a per-property basis through the `@export_custom` hint string as discussed below.

For custom list-creation behavior, you can 
...
Except when using the `options` hint, the default filler only finds `const StringName` values to fill its list of words (but it can be used with properties of both `String` and `StringName` type). 

Godot supports providing the editor information about your exported variables through `@export_custom`. For this plugin, the desired line for non-nested `Strings` or `StringNames` is: 
```
...
- `options:Option1,Option2,etc` Ignores the directory file. Mimics `@export_enum` (in a way) by loading the explicitly provided comma separated list of strings (does not strip spaces). Unlike `@export_enum`, this does support `StringName`, not just `String`.
...
NOTE: Everything is case sensitive, but of course you can decide the case of your members and list names.
NOTE: I don't think the PropertyHint matters at all, but I may be wrong. 

### Using a directory file with the default filler
...
### Editing nested data in Array or Dictionary types:
...
```
successfully loads all strings associated with `LIST=Tags`. For dictionary types, Godot separates the key and value with `;` but both still need the `:` in front, so the following works:
```
...
- Directory File Path: Path to the file that tells the `StringRegistryFiller` which files to look in. Defaults to the directory in this plugin's example directory. This can have any necessary data for custom implementations.
...
- Add option `always_on` to toggle on/off whether by default the plugin should try to work for every string/stringname.
  ...
  - This option is off by default because Strings and StringNames are edited within the engine in so many places that it seems most prudent to only load the plugin in cases where the user specifically desires it, especially given that the user will likely want to use `@export_custom` for finer control is most cases, anyway. Using `always_on` increases the likelihood of unexpected problematic behavior in the editor.

## Implementing Custom Fillers
See the documentation in `StringRegistryFiller` for extending that class and see `ConstStringNameRegistryFiller` as an example. The filler class receives the object being edited, the hint_string provided in the custom export and the location of the directory file and may use that as it wishes to create a list. When an empty list is returned, the plugin silently refuses to load. 
...
- Fixed check-mark to reposition correctly when the inspector is resized.
- Added examples of usage with Arrays or Dictionaries
